...
Candidates are compared only within the same canonical field concept and semantic role. For example, `金额`, `金额（RMB）`, and `金额（USD）` compete with one another, while `店铺` and `金额` are resolved independently. Every confirmed or assumed field is stored in the step and top-level execution plan as a required source column. This is a step-level field contract shared by every downstream execution path, not only code generation: deterministic rules use it when choosing filter, date, sort, grouping, and metric columns; chart planning uses matching result columns as the allowed dimension/time and metric axes. A field used only as a filter input does not have to become a visible chart axis. A follow-up containing an exact candidate column resolves the ambiguity and resumes the original analysis.
